...
static void reset_flash(void)
{
	...
	HAL_SPI_Transmit(NULL, &cmd, 1, 100);
...
void app_flash_initialize(void)
{
    ...
    if (flash_self_test())
    {
		...
		case W25Q128:
			...
			break;
        default:
            DEBUG_INFO("UNKNOWNN: %u", m_flash_version);
            break;
        }
		DEBUG_RAW("\r\n");
        m_flash_inited = true;
    ...
    else
    {
        ...
        DEBUG_INFO("Khoi tao Flash: [FAIL]\r\n");
        return;
    }
...
static void wait_write_in_process(uint32_t addr)
{
    ...
    while (timeout)
    {
        timeout--;
        cmd = SPI_DUMMY;
		HAL_SPI_Receive(NULL, &cmd, 1, 100);
...
//        /* Gui 4 byte dia chi */
    ...
    /* Gui du lieu */
    for (i = 0; i < length; i++)
        HAL_SPI_Transmit(NULL, buffer + i, 1, 100);
...
//        FLASH_SPI_SendByte(READ_DATA_CMD);
	...
	cmd = SPI_DUMMY;
    /* Lay du lieu */
	HAL_SPI_Receive(NULL, buffer, length, 100);
...
uint8_t flash_self_test(void)
{
    ...
    for (loop_count = 0; loop_count < 3; loop_count++)
    {
		...
		cmd = 0xFF;
		HAL_SPI_TransmitReceive(NULL, &cmd, &manufacture_id, 1, 100);
        HAL_SPI_TransmitReceive(NULL, &cmd, &device_id, 1, 100);
        ...
        DEBUG_INFO("device id: %X, manufacture id: %X\r\n", device_id, manufacture_id);
		...
		else if (manufacture_id == 0xEF && device_id == 0x17) /* FL256S - 1 chan CS 256Mb */
		{
			...
			// DEBUG_INFO("status register: %02X", reg_status);
			// if (reg_status & 0x01)
			// {
			//     DEBUG_INFO("Da vao che do 32 bits dia chi");
			// }
			reset_flash();
			enable_ext_flash(0);
		}
        else if (manufacture_id == 0xC8 && device_id == 0x18) /* GD256C - GigaDevice 256Mb */
        {
			...
			cmd = SPI_DUMMY;
			HAL_SPI_TransmitReceive(NULL, &cmd, &reg_status, 1, 100);
            enable_ext_flash(0);
...
//        ResetWatchdog();
		cmd = SPI_DUMMY;
        HAL_SPI_TransmitReceive(NULL, &cmd, &status, 1, 100);
        if ((status & 1) == 0)
            break;
        sys_delay_ms(50);
    }
...
uint32_t app_flash_estimate_next_write_addr(void)
{
	...
	{
		m_wr_addr = PAGE_SIZE;
		DEBUG_INFO("Flash full, erase first block\r\n");
