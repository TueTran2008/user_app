...
bool ota_update_start(uint32_t expected_size)
{
    ...
    {
        DEBUG_INFO("Firmware size too large %ubytes, max allowed %ubytes", 
                    expected_size, 
                    OTA_MAX_SIZE);
        return false;
    }
...
bool ota_update_write_next(uint8_t *data, uint32_t length)
{
        ...
        && m_current_write_size < (strlen(OTA_UPDATE_DEFAULT_HEADER_DATA_FIRMWARE) + strlen(OTA_UPDATE_DEFAULT_HEADER_DATA_HARDWARE)))
    {
        ...
        if (!p)
        {
            ...
            ota_update_set_invalid_binary_firmware();
            NVIC_SystemReset();
            return false;
        }
        ...
        uint32_t nb_of_page = (m_expected_size + OTA_UPDATE_DEFAULT_HEADER_SIZE + FLASH_PAGE_SIZE - 1) / FLASH_PAGE_SIZE;
    ...
    while (length)
    {
        ...
        if (m_ota_remain.size == OTA_REMAIN_MAX_LENGTH)
        {
            DEBUG_VERBOSE("Write %u bytes, at 0x%08X\r\n", 
                            m_ota_remain.size, 
                            DONWLOAD_START_ADDR + m_current_write_size);
            flash_if_write(DONWLOAD_START_ADDR + m_current_write_size, 
                            (uint32_t *)&m_ota_remain.data[0], 
                                m_ota_remain.size / sizeof(uint32_t));
...
#else
        DEBUG_INFO("Write OTA info page\r\n");
        ota_flash_cfg_t new_cfg;
...
void ota_update_rollback_firmware(void)
{
        ota_flash_cfg_t new_cfg;
        new_cfg.flag = OTA_UPDATE_FLAG_ROLLBACK_FIRMWARE;
...
void ota_update_set_invalid_binary_firmware(void)
{
        ota_flash_cfg_t new_cfg;
        new_cfg.flag = OTA_UPDATE_FLAG_INVALID_BINARY_FIRMWARE;
        new_cfg.firmware_size = 0;
        new_cfg.reserve[0] = 0;
        flash_if_write_ota_info_page((uint32_t *)&new_cfg, 
                                    sizeof(ota_flash_cfg_t)/sizeof(uint32_t));
