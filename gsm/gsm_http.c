...
void gsm_http_download_big_file(gsm_response_event_t event, void *response_buffer)
{
    ...
    if (m_http_read_big_file_step == 0)
    {
        ...
        DEBUG_INFO("Speed %ukb/s, download time %ums\r\n", 
                    (m_content_length * 1000 / 1024) / download_time, 
                    download_time);
...
//                DEBUG_VERBOSE("Post total %u msg\r\n", DEBUG_HTTP_POST_GET_COUNT());
    ...
    case 7: // Set url
    {
        DEBUG_VERBOSE("SSL : %s, response %s\r\n",
                   (event == GSM_EVENT_OK) ? "[OK]" : "[FAIL]",
                   (char *)response_buffer);
        DEBUG_INFO("URL %s\r\n", m_http_cfg.url);
        if (event == GSM_EVENT_OK)
    ...
    case 9: // Start HTTP download
    {
        ...
        else // POST
        {
            ...
            sprintf(m_http_cmd_buffer, "AT+QHTTPPOST=%d,3,7\r\n",
                    (int)post_rx_data.data_length);
            gsm_hw_send_at_cmd(m_http_cmd_buffer,
                               "CONNECT",
                               "",
                               11000,
    ...
    case 10:
    {
        DEBUG_VERBOSE("HTTP action : %s, response %s\r\n",
                   (event == GSM_EVENT_OK) ? "[OK]" : "[FAIL]",
                   (char *)response_buffer);
        ...
        else if (m_http_cfg.action == GSM_HTTP_ACTION_GET) // GET
        {
            if (event == GSM_EVENT_OK)
            {
                                                             ...
                                                             && m_content_length)
                {
                    ...
                    DEBUG_VERBOSE("Content length %u\r\n", m_content_length);
                    if (!m_http_cfg.big_file_for_ota)
                    {
