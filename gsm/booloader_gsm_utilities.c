...
bool gsm_utilities_get_storage_file_info(char *buffer, uint32_t *size, uint32_t *number_of_file)
{
    ...
    {
        memset(tmp, 0, sizeof(tmp));
        gsm_utilities_copy_parameters(p, tmp, ' ', ','); 
        
        // Get free space
        if (size)
            *size = gsm_utilities_get_number_from_string(0, tmp);
        
        // Get number of file
        if (number_of_file)
            *number_of_file = gsm_utilities_get_number_from_string(1, q);
