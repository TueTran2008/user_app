...
/**
 * @brief                       Parse GSM SNTP timestamp response message
 * @param[in]                   buffer : Pointer to buffer data will be parsed
 * @param[out]                  year : Year from 2000
 * @param[out]                  month : Month
 * @param[out]                  month_day : Month day
 * @param[out]                  hour : Hour
 * @param[out]                  minute : Minute
 * @param[out]                  second : Second
 * @note :                      Format message "+QNTP: 0,"2021/08/29,07:32:01+28\r\n"
 * @retval                      TRUE  : Operation success
 *                              FALSE : Response message is invalid format or invalid timestamp
 */
bool gsm_utilities_parse_sntp_timestamp_buffer(char *response_buffer, 
                                                uint8_t *year, uint8_t *month, uint8_t *month_day, 
                                                uint8_t *hour, uint8_t *minute, uint8_t* second);

/**
 * @brief                       Parse GSM QIACT polling state
 * @param[in]                   buffer : Pointer to buffer data will be parsed
 * @param[out]                  retval : QIACT result
 * @note :                      Format message "+QIACT: <context_id>,<context_state>,<context_type>,<IP_address>", QIACT: 1,1,1,"100.92.227.8\r\nOK\r\n
 * @retval                      TRUE  : Operation success
 *                              FALSE : Response message is invalid format or invalid timestamp
 */
bool gsm_utilities_parse_qiact_query_response(char *response_buffer, gsm_utilites_qiact_state_t *retval);

/**
 * @brief                       Parse GSM QFile storeage info
 * @param[in]                   buffer : Pointer to buffer data will be parsed
 * @param[out]                  size : Storage free space
 * @param[out]                  number_of_file : Storage number of file
 * @note :                      Format message "+QFLDS: 40164800,63768576\r\nOK"
 * @retval                      TRUE  : Operation success
 *                              FALSE : Response message is invalid
 */
bool gsm_utilities_get_storage_file_info(char *buffer, uint32_t *size, uint32_t *number_of_file);
