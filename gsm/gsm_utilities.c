...
bool gsm_utilities_parse_sntp_timestamp_buffer(char *response_buffer, 
                                                uint8_t *year_from_2k, uint8_t *month, uint8_t *month_day, 
                                                uint8_t *hour, uint8_t *minute, uint8_t* second)
{
    // +QNTP: 0,"2021/08/29,07:32:01+28
    bool val = false;
    uint8_t tmp[5];
    int year;
    char *p_index = strstr(response_buffer, "+QNTP: ");
    if (p_index == NULL)
    {
        goto exit;
    }
    p_index += 6;    // 6 = stren "+QNTP: "
    while (*p_index && ((*p_index) != '"'))
    {
        p_index++;
    }
    ...
    {
        *year_from_2k = year - 2000;

        // MM
        response_buffer += 5;       // 5 = strlen("2021/")
        memset(tmp, 0, sizeof(tmp));
        memcpy(tmp, response_buffer, 2);
        *month = atoi((char*)tmp);

        // dd
        response_buffer += 3;       // 3 = strlen("08/")
        memset(tmp, 0, sizeof(tmp));
        memcpy(tmp, response_buffer, 2);
        *month_day = atoi((char*)tmp);

        // hh
        response_buffer += 3;   // 4 = strlen("29,")
        memset(tmp, 0, sizeof(tmp));
        memcpy(tmp, response_buffer, 2);
        *hour = atoi((char*)tmp);
        ...
        memset(tmp, 0, sizeof(tmp));
        memcpy(tmp, response_buffer, 2);
        *minute = atoi((char*)tmp);

        // ss
        response_buffer += 3;
        memset(tmp, 0, sizeof(tmp));
        memcpy(tmp, response_buffer, 2);
        *second = atoi((char*)tmp);
...
bool gsm_utilities_parse_qiact_query_response(char *response_buffer, gsm_utilites_qiact_state_t *retval)
{
    ...
    uint32_t i = 0;
    uint32_t nb_of_comma = 0;
    uint32_t ip = 0;
    char *p = response_buffer;
    char *tmp = NULL;
    
    if (!response_buffer || !retval)
    {
        goto end;
    }

    p = strstr(response_buffer, "QIACT: ");
    if (p == NULL || strstr(p, "OK\r\n") == NULL)
    {
        goto end;
    }
