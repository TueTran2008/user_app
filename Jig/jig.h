...
/**
 * @brief		Set JIG test mode
 * @param[in]   TRUE Jig is in test mode
 *              FALSE JIG is not in test mode
 */
void jig_set_test_mode(bool set);

/**
 * @brief		Check test cmd
 * @retval		TRUE jig new test cmd data found, device need to enter test mode
 *              FALSE jig new test data not found
 */
bool jig_found_cmd_test(void);

/**
 * @brief		Get shutdown request
 * @retval		TRUE Host request device enter shutdown mode
 *              FALSE Not found shutdown command
 */
bool jig_found_cmd_shutdown(void);

/**
 * @brief		Get reboot request
 * @retval		TRUE Host request device reboot
 *              FALSE Not found reboot command
 */
